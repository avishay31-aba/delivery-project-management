...
For Codex Cloud execution, a platform-provided temporary branch such as `work` is permitted and need not have the authoritative branch name. Before implementation, verify through the available GitHub/platform state that it was materialized from the latest approved `release/v1.2-final-gap-corrections` checkpoint. Refresh or restart through the supported mechanism if it is stale. A temporary Cloud branch is never authoritative. If its relationship to the authoritative checkpoint cannot be safely established, stop and report the ambiguity.
