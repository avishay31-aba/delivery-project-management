...
Tenant fields include TID and existing tenant context. There is no Tenant Name business field.
...
- Opportunity ID generated by the application at Save and unique.
...
Customer Existing Tenants / Systems is Tenant-driven. It lists only eligible Customer Tenants belonging to the Opportunity Account whose current warranty header status is `Under Contract` or `Not Set Yet`; it excludes POC Tenants, Internal Tenants, ineligible Customer Tenants, and system-only rows. The displayed SID resolves from the Tenant's current hosting System. Grid B and Grid C use the same eligible Customer Tenant set. Grid A is the only grid that may select an existing System; its existing-System options are deduplicated from Systems currently hosting eligible Customer Tenants for the Opportunity Account, followed by eligible Systems from other Accounts under the same Sales Manager / Deal Owner. Standalone Systems are valid inventory, but they do not appear in this Opportunity table merely because they exist.
