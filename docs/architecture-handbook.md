...
- Recoverable business-object Delete/Restore implementations must reuse the Project deletion pattern unless an explicit domain rule requires otherwise: delete from the active dashboard; active/deleted dashboard separation; View/Edit/Delete and View/Edit/Restore row actions; authoritative deletion history with Deleted By, timestamp, and rich-text reason; newest-first header history with dotted separators; current reason editable only while currently deleted; restore preserving history and restoring the previous legitimate lifecycle/status.
- Infrastructure Item deletion applies this pattern with system-controlled `Operational Status = Deleted` as the deleted state. `Obsolete` remains a manually selected operational status with independent business meaning. Restore returns the item to the exact pre-delete operational status when legitimate, falling back to `Active` only when no valid prior status exists.
