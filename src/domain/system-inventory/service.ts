...
export function systemTimeGroup(record: SystemInventoryRecord, tenants: Tenant[], timeGroupLookups: TimeGroupLookupRecord[] = []): string {
  ...
  return systemTimeGroupFromVeteranTenant(record.id, tenants, timeGroupLookups, 'timeGroupOverrideTenantId' in record ? record.timeGroupOverrideTenantId : null).timeGroup
}
