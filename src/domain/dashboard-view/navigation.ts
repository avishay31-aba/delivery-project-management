...
export function dashboardRecordRoutePath(scope: DashboardViewScope, row: unknown): string | null {
  ...
  switch (scope) {
    ...
    case 'systems':
    case 'cancelledSystems':
      if (text(record.id).startsWith('reused-sys-')) return routePathForBusinessReference('INTERNAL_REUSED_SYSTEM', text(record.machineId) || text(record.mid) || text(record.id))
      if (text(record.id).startsWith('prod-sys-')) return routePathForBusinessReference('PRODUCTION_SYSTEM', text(record.sid) || text(record.id))
