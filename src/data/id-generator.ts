...
export function deriveIdCountersFromRecords(
  ...
  const derived: IdCounters = {
    ...
    infrastructureManufacturer: maxCounter((state.referenceData ?? []).filter((record) => record.referenceType === 'INFRASTRUCTURE_MANUFACTURER').map((record) => record.id), 'infrastructureManufacturer'),
    infrastructureOwner: maxCounter((state.referenceData ?? []).filter((record) => record.referenceType === 'INFRASTRUCTURE_OWNER').map((record) => record.id), 'infrastructureOwner'),
    infrastructureBillingMethod: maxCounter((state.referenceData ?? []).filter((record) => record.referenceType === 'INFRASTRUCTURE_BILLING_METHOD').map((record) => record.id), 'infrastructureBillingMethod'),
    infrastructureWarrantyType: maxCounter((state.referenceData ?? []).filter((record) => record.referenceType === 'INFRASTRUCTURE_WARRANTY_TYPE').map((record) => record.id), 'infrastructureWarrantyType'),
    infrastructurePropertyValue: maxCounter((state.referenceData ?? []).filter((record) => record.referenceType === 'INFRASTRUCTURE_PROPERTY_VALUE').map((record) => record.id), 'infrastructurePropertyValue'),
    infrastructureMaintenanceTask: maxCounter((state.infrastructureItems ?? []).flatMap((item) => (item.maintenanceTasks ?? []).map((task) => task.taskId)), 'infrastructureMaintenanceTask'),
    pid: 0,
    sid: 0,
    tid: 0,
